package cc.allyapps.pastely;

import co.elastic.clients.elasticsearch.ElasticsearchClient;
import com.google.gson.Gson;
import cc.allyapps.pastely.ai.PasteAI;
import cc.allyapps.pastely.auth.*;
import cc.allyapps.pastely.auth.strategies.oauth2.OAuth2Strategy;
import cc.allyapps.pastely.auth.strategies.oauth2.providers.*;
import cc.allyapps.pastely.cli.PastelyCLI;
import cc.allyapps.pastely.controller.AppController;
...
import cc.allyapps.pastely.model.plugins.PastelyBackendPlugin;
import cc.allyapps.pastely.model.plugins.PastelyPlugin;
import cc.allyapps.pastely.model.responses.ExceptionResponse;
...
public class Pastely {

    private static Pastely instance;
    private JedisPool redisPool;

    public static Pastely getInstance() {
        return instance;
    }
    ...
    private final Map<String, PastelyPlugin> plugins = new HashMap<>();

    public final String[] INDEXES = new String[]{"pastely_pastes"};
    ...
    protected void setupConfig() {
        Mapping mapping = new Mapping()
                ...
                .map("PASTELY_GRANT_ACCESS_REQUIRED", "pastely.grantaccessrequired")
                .map("PASTELY_LOGIN_REQUIRED", "pastely.loginrequired")
                .map("PASTELY_LOGIN_REQUIRED_CREATE", "pastely.loginrequired.create")
                .map("PASTELY_LOGIN_REQUIRED_READ", "pastely.loginrequired.read")
                .map("PASTELY_LIST_PASTES", "pastely.listpastes")
                .map("PASTELY_PUBLIC_STATS", "pastely.publicstats")
                .map("PASTELY_PUBLIC_PASTES", "pastely.publicpastes")
                .map("PASTELY_META_TAGS", "pastely.metatags")
                .map("PASTELY_CUSTOM_BODY", "pastely.custombody")
                .map("PASTELY_CUSTOM_HEADER", "pastely.customheader")
                .map("PASTELY_PAGINATION_PAGE_LIMIT", "pastely.paginaton.pagelimit")
                .map("PASTELY_DEV", "pastely.dev")

                .map("PASTELY_AUTOMIGRATE", "pastely.automigrate")
        ...
        loginRequired = config.get("pastely.loginrequired", "false").toLowerCase(Locale.ROOT).equals("true");
        loginRequiredForCreate = loginRequired || config.get("pastely.loginrequired.create", "false").toLowerCase(Locale.ROOT).equals("true");
        loginRequiredForRead = loginRequired || config.get("pastely.loginrequired.read", "false").toLowerCase(Locale.ROOT).equals("true");
        publicPastesEnabled = config.get("pastely.publicpastes", "true").toLowerCase(Locale.ROOT).equals("true");
    }
    ...
    protected void initPlugins() {
        ...
        for (File file : pluginsFolder.listFiles()) {
            ...
            try {
                PastelyPlugin pastelyPlugin = new PastelyPlugin(pluginFolder.getPath());
                System.out.println("Registered plugin " + pastelyPlugin.config.name);
                plugins.put(pastelyPlugin.config.name, pastelyPlugin);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    protected void setupModels() {
        ...
        getBackendPlugins().forEach(PastelyBackendPlugin::setupModels);
    }
    ...
    protected void setupPassport() {
        ...
        getBackendPlugins().forEach(PastelyBackendPlugin::setupPassport);
    ...
    public void setupServer() {
        ...
        httpRouter.beforeInterceptor(exchange -> {
            exchange.header("Server", "InteraApps-Pastely");
        ...
        List<PastelyPlugin> list = plugins.values().stream().filter(p -> p.config.frontend != null).collect(Collectors.toList());
        list.forEach(p -> {
        ...
        try {
            ...
            String customHeaders = config.get("pastely.customheader", "");
            String customBody = config.get("pastely.custombody", "");

            homeHTML = homeHTML
                    .replace("/*PASTELY_PLUGINS*/", registerPlugins.toString())
        ...
        httpRouter.ws("/api/v2/ws/collaboration/{sessionId}", new cc.allyapps.pastely.websocket.CollaborationWebSocketHandler());
        ...
        getBackendPlugins().forEach(PastelyBackendPlugin::setupServer);
    }
    ...
    public static void main(String[] args) {
        instance = new Pastely();
        int exitCode = new CommandLine(new PastelyCLI()).execute(args);
    ...
    public List<PastelyBackendPlugin> getBackendPlugins() {
    ...
    public boolean isDevMode() {
        return config.get("pastely.dev", "false").toLowerCase(Locale.ROOT).equals("true");
    }
