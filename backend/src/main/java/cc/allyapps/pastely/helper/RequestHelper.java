...
import cc.allyapps.pastely.model.database.PasteTag;
...
public class RequestHelper {
    ...
    public static void pagination(Query<?> query, Exchange exchange) {
        ...
        if (Pastely.getInstance().getConfig().has("pastely.paginaton.pagelimit")) {
            int configLimit = Pastely.getInstance().getConfig().getInt("pastely.paginaton.pagelimit", 50);
    ...
    public static void userIdPastesFilter(User user, Query<?> query, Exchange exchange) {
        ...
        if (user == null && !Pastely.getInstance().getConfig().get("pastely.listpastes", "false").equalsIgnoreCase("true"))
