...
import cc.allyapps.pastely.auth.strategies.oauth2.OAuth2Profile;
...
public class OAuth2Callback implements OAuth2Strategy.HttpCallbackHandler {
    public Object handle(Exchange exchange, cc.allyapps.pastely.auth.strategies.oauth2.OAuth2Callback callback, String name) {
        User.AuthenticationProvider provider = cc.allyapps.pastely.model.database.User.AuthenticationProvider.getProviderByClass(callback.getProvider().getClass());
        ...
        if (user == null) {
            user = new cc.allyapps.pastely.model.database.User();
            int i = 1;
            final String uniqueName = profile.getName().replaceAll("[^a-zA-Z0-9]", "");
            user.uniqueName = uniqueName;
            while (Repo.get(cc.allyapps.pastely.model.database.User.class).where("uniqueName", user.uniqueName).first() != null) {
                user.uniqueName = uniqueName + i++;
            }
            user.authId = profile.getId();
            user.authProvider = provider;

            if (Pastely.getInstance().getConfig().get("pastely.grantaccessrequired", "false").equalsIgnoreCase("true"))
