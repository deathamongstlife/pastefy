...
import de.interaapps.pastely.model.elastic.ElasticPaste;
import de.interaapps.pastely.model.minio.MinioPaste;
import de.interaapps.pastely.model.redis.PasteAccessCache;
import de.interaapps.pastely.model.redis.PasteContentCache;
...
public class Paste extends Model {
    ...
    public void writeResponse(Exchange exchange) throws IOException {
        ...
        if (Pastely.getInstance().isMinioEnabled() && storageType == StorageType.S3) {
    ...
    public void addTag(String tag) {
        ...
        Pastely.getInstance().executeAsync(() -> ElasticPaste.updateTags(this));
    }
    ...
    public void save() {
        ...
        if (Pastely.getInstance().isElasticsearchEnabled()) {
            Pastely.getInstance().executeAsync(() -> MinioPaste.store(this));
        }
    ...
    public void delete() {
        ...
        if (storageType == StorageType.S3) {
            Pastely.getInstance().executeAsync(() -> MinioPaste.delete(this));
        }
        if (indexedInElastic != null && indexedInElastic) {
            Pastely.getInstance().executeAsync(() -> ElasticPaste.delete(this));
        }
