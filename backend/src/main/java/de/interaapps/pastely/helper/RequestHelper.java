package de.interaapps.pastely.helper;
...
import de.interaapps.pastely.model.database.PasteTag;
...
public class RequestHelper {
    ...
    public static void pagination(Query<?> query, Exchange exchange) {
        ...
        if (Pastely.getInstance().getConfig().has("pastefy.paginaton.pagelimit")) {
            int configLimit = Pastely.getInstance().getConfig().getInt("pastefy.paginaton.pagelimit", 50);
    ...
    public static void userIdPastesFilter(User user, Query<?> query, Exchange exchange) {
        ...
        if (user == null && !Pastely.getInstance().getConfig().get("pastefy.listpastes", "false").equalsIgnoreCase("true"))
