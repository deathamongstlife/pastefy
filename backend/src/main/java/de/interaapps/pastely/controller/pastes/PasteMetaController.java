...
public class PasteMetaController extends HttpController {
    ...
    public String pasteTags(Exchange exchange, @Path("paste") String pasteId) {
        if (!Pastely.getInstance().getConfig().get("pastefy.metatags", "false").equalsIgnoreCase("true")) return null;
        ...
        String url = Pastely.getInstance().getConfig().get("server.name") + "/" + pasteId;
        tags.put("og:url", url);
