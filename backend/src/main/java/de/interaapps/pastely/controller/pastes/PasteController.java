package de.interaapps.pastely.controller.pastes;
...
import de.interaapps.pastely.exceptions.PastePrivateException;
...
import de.interaapps.pastely.model.requests.paste.AddFriendToPasteRequest;
import de.interaapps.pastely.model.requests.paste.CreatePasteRequest;
import de.interaapps.pastely.model.requests.paste.EditPasteRequest;
import de.interaapps.pastely.model.responses.ActionResponse;
import de.interaapps.pastely.model.responses.paste.CreatePasteResponse;
    ...
    public CreatePasteResponse create(Exchange exchange, @Body CreatePasteRequest request, @Attrib("user") User user, @Attrib("authkey") AuthKey authKey) {
        ...
        if (request.forkedFrom != null) {
            ...
            if (forkedFrom != null && forkedFrom.isPublic()) {
                Pastely.getInstance().executeAsync(() -> PublicPasteEngagement.addInterestFromPaste(forkedFrom, 10));
            }
        }
        ...
        if (request.ai && !request.encrypted && Pastely.getInstance().aiEnabled()) {
            Pastely.getInstance().executeAsync(() -> {
                try {
                    AtomicInteger count = new AtomicInteger();
                    AbstractObject aiResponse = Pastely.getInstance().getPasteAI().generateTags(paste);
